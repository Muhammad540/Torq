...
| **RobotSystem** | User-added tasks, limits, barriers | Objects passed to `addTask()` / `addLimit()` / `addBarrier()` are owned by RobotSystem and destroyed on `remove*()` or when RobotSystem is destroyed. |
...
- @b torq::Task @b (abstract) — defines `computeError()`, `computeJacobian()`, `computeQPObjective()`.
  ...
  - @b torq::ComTask @b, @b torq::RelativeFrameTask @b, @b torq::LinearHolonomicTask @b, @b torq::JointCouplingTask @b, @b torq::JointVelocityTask @b, @b torq::LowAccelerationTask @b — add via RobotSystem::addTask() for custom IK (e.g. humanoids, quadrupeds).
...
- @b torq::Limit @b (abstract) — defines `computeQPInequalities()`.
  ...
  - @b torq::FloatingBaseVelocityLimit @b, @b torq::AccelerationLimit @b — optional built-in or add via RobotSystem::addLimit().
